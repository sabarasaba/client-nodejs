(function() {
	...
	function Request(method, url, options) {
		...
		var requestObject = {
			...
			capturedRequestKey = Object.keys(capturedRequests).length + ': ' + method + ' ' + url;

		capturedRequests[capturedRequestKey] = {
			req: requestObject,
			res: null
		};

		requestQueue.push(requestObject, (function(err, meta, body) {
			this.response = {
			...
			capturedRequests[capturedRequestKey].res = {
				body: body,
				meta: meta,
				err: err
			};
